...
render_nft() {
    ...
    umask 022
    {
        ...
        printf '# Policy route: table=%s priority=%s mark=%s\n' "$ROUTE_TABLE" "$RULE_PRIORITY" "$TPROXY_MARK"
...
policy_current() {
    [ -L "$NFT_INCLUDE_LINK" ] && [ "$(readlink "$NFT_INCLUDE_LINK")" = "$NFT_RUNTIME_FILE" ] &&
        cmp -s "$1" "$NFT_RUNTIME_FILE" &&
        nft list chain inet fw4 xray_prerouting >/dev/null 2>&1
}

policy_unchanged() {
    acquire_lock
    temp="/tmp/30-xray-router.nft.compare.$$"
    trap 'rm -f "$temp"; rmdir "$LOCK_DIR" 2>/dev/null || true' EXIT HUP INT TERM
    render_nft "$temp"
    result=0
    policy_current "$temp" || result=1
    rm -f "$temp"
    release_lock
    return "$result"
}
...
policy_up() {
    [ "$#" -le 1 ] || die "Usage: policy.sh up [--no-check|--if-needed]"
    case "${1:-}" in ''|--no-check|--if-needed) ;; *) die "Usage: policy.sh up [--no-check|--if-needed]" ;; esac
    require_command fw4
    require_command ip
    acquire_lock
    ...
    render_nft "$temp"
    if [ "${1:-}" = --if-needed ] && policy_current "$temp"; then
        ensure_policy_route
        rm -f "$temp"
        release_lock
        log "transparent interception unchanged; no firewall reload needed"
        return 0
    fi
...
main() {
    ...
    case "$command" in
        ...
        *)
            printf '%s\n' "Usage: $0 {render [FILE]|check|unchanged|up [--no-check|--if-needed]|down|status}" >&2
            exit 2
            ;;
    esac
}
