...
return view.extend({
	...
	render: function(data) {
			...
			ui.showModal(_('Change logging and restart Xray'), [
				...
				E('div', { 'class': 'right' }, [E('button', { 'class': 'cbi-button', 'click': ui.hideModal }, _('Cancel')), ' ',
					E('button', { 'class': 'cbi-button cbi-button-negative', 'click': function() {
						ui.hideModal(); jobBusy = true; ownLogging = true; update();
		...
		var root = E('div', { 'class': 'cbi-map' }, [
			...
			E('details', { 'class': 'cbi-section' }, [E('summary', {}, _('Logging settings (may restart Xray)')), logStatus, logNotices,
			...
			E('p', {}, E('a', { 'href': L.url('admin/services/xray-router') }, _('Open Xray Router settings')))
		]);
		update();
		poll.add(function() {
			return Promise.all([getCapture(), getJob()]).then(function(values) {
				...
				if (ownLogging && !jobBusy) {
					...
					return options().then(checked).then(function(next) { info = next; update(); });
				}
				update();
